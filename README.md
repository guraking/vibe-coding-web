...
- **자연어 → 코드 생성** — Groq API(LLaMA, Mixtral)를 이용해 멀티 파일 웹 프로젝트를 자동 생성
...
| 빌드 도구 | Vite 6 |
| 스타일링 | Tailwind CSS |
| AI API | [Groq](https://groq.com) (llama-3.3-70b, llama-3.1-8b, mixtral-8x7b) |
| 아이콘 | lucide-react |
...
[Groq Console](https://console.groq.com)에서 API 키를 발급받은 후 `.env.local` 파일을 생성합니다.

```env
VITE_GROQ_API_KEY=gsk_your_key_here
```

> `.env.local`이 없으면 앱 실행 후 설정 버튼에서 직접 입력할 수 있습니다.
...
1. 우측 상단 설정에서 Groq API 키와 모델을 선택합니다.
2. 채팅 입력창에 만들고 싶은 앱을 한국어 또는 영어로 설명합니다.
3. 우측 미리보기 패널에서 결과를 확인하고, 추가 수정 요청을 입력합니다.
4. 완성된 프로젝트는 내보내기(Export) 버튼으로 ZIP 파일로 다운로드할 수 있습니다.
